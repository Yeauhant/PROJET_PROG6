...
/*
 * Classe ConfigVector
 * Construit le vecteur de bits associ�s � la configuration donn�e en argument.
 * N�cessite un plateau et l'id du joueur courant :
...
public class ConfigVector {
	...
	private void Assemble(Board Game, int turn){
		...
		while (l >= 0 || c < Game.get_width()){
			/* On est tout en haut. On va � droite. */
			if(l < 0){
				vect.add(0);
				c++;
			}
			/* On est tout � droite. On va en haut. */
			else if (c >= Game.get_width()) {
				vect.add(1);
				l--;
			}
			/* Sinon, on regarde s'il y a un carr� de gaufre ici. */
			else {
